...
def map_gene_ids(df, gene_to_fbgnid_main, gene_to_fbgnid_synonym, gene_col):
    ...
    steps = ['lowercase', 'remove_zeros', 'remove_hyphen', 'number_to_end'] # 'add_cr'
...
if __name__ == "__main__":
    ...
    mappings_df = pd.read_csv('~/Desktop/Projects.nosync/Allada-Lab/Fly_Stock_Scrapers/Data/FlyBase/Genes/fb_synonym_fb_2024_06.tsv', sep='\t', header=0, low_memory=False, dtype=columns_as_strings)
    ...
    for excel_file in excel_files:
        ...
        if gene_column not in df.columns:
            print(f"'{gene_column}' column not found in {excel_file}. Skipping file.")
        ...
        if ("GO" in excel_file and "Genes" in df.columns):
        ...
        print(f"File saved to {excel_file} with {df.shape[0]} rows.")
